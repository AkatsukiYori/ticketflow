...
export default function TicketDetailModal({ open, data, onClose }: Props) {
    ...
    const fetchFeedback = useCallback(async () => {
        try {
            return await callApi("get", `/tickets/get-feedback-tickets/${data?.ticket_no}`);
        } catch (error: any) {
            Notifications({ message: "Failed to fetch data.", variantType: 'error', persist: false });
        }
    }, [callApi, data?.ticket_no]);

    const { data: feedbackData } = useQuery({
        queryKey: ['feedback', data?.ticket_no],
        queryFn: fetchFeedback,
        refetchOnWindowFocus: true,
        refetchOnMount: true,
        staleTime: 0,
        enabled: !!data?.ticket_no
    });
    ...
    return (
        <div className={`${Styles['modal-overlay']} ${open ? Styles['modal-overlay-show'] : "hide"}`}>
            <div className={`${Styles['modal-popup']} ${open ? Styles['modal-popup-show'] : "hide"}`}>
                ...
                <div className={Styles['modal-body']} style={{ display: "flex", flexDirection: "row" }}>
                    <div style={{ flex: 2 }}>
                                    ...
                                    {feedbackData.map((item: any) => (
                                        <>
                                            <div>
                                                {item.user_id ? (
                                                    <p style={{margin: 0 }}>
                                                        {item.fk_admin_id?.username} <span>{new Date(item.created_at).toLocaleString("id-ID", {
                                                            day: "2-digit",
                                                            month: "long",
                                                            year: "numeric",
                                                            hour: "2-digit",
                                                            minute: "2-digit",
                                                            timeZone: "Asia/Jakarta"
                                                        }).replaceAll(" pukul", ",").replaceAll(".", ":")}</span>
                                                    </p>
                                                ) : (
                                                    <p style={{margin: 0 }}>
                                                        {data?.fk_member?.username} <span>{new Date(item.created_at).toLocaleString("id-ID", {
                                                            day: "2-digit",
                                                            month: "long",
                                                            year: "numeric",
                                                            hour: "2-digit",
